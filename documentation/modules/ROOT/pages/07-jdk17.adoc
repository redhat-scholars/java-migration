...
Then create the first sub-class of `Animal` named `Reptile` sealed to `Lizard` and `Turtle` wihch means that only `Lizard` and `Turtle` classes can extend `Reptile` class.
